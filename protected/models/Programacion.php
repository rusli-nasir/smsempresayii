...
class Programacion extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('asunto, fechaInicio, fechaFin, activo', 'required'),
			array('activo', 'numerical', 'integerOnly'=>true),
			array('asunto', 'length', 'max'=>150),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, asunto, fechaInicio, fechaFin, activo', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		return new CActiveDataProvider('Programacion', array(
			'criteria'=>$criteria,
		));
	}
}